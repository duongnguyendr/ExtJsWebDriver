...
public class ComboBox extends ExtJSComponent
{
    ...
    static final String setComboBox = "ExtJsWebDriver.prototype.setComboBox = function (query, item, uuid) {" + "var success = false;"
                    + "var comp = this.findVisibleComponent(query);" + "var store = comp.getStore();"
                    + "var index = store.find(comp.displayField, item);" + "if ( index != -1 ) {" + "	comp.setValue(store.getAt(index));"
                    + "	success = comp.fireEvent('select', comp, [store.getAt(index)]);" + "}" + "writeDataToDiv(success, uuid);" + "}";
