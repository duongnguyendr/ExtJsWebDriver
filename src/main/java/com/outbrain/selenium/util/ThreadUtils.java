...
public class ThreadUtils {
	...
	public static boolean waitFor(final WaitCondition condition) {
		...
		if (condition != null) {
			...
			while (!(result = condition.checkCondition(System.currentTimeMillis()
					- startTime))) {
				try {
					Thread.sleep(100);
				} catch (final InterruptedException ex) {
	...
	public static boolean waitFor(final WaitCondition condition, final TimeUnit timeUnitTimeout, final long timeoutDuration, final TimeUnit timeUnitSleep,
